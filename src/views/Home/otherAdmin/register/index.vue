<template>
	<div>
		...
		<div class="content">
			...
			<accep-unit v-if="key == '4'" :datas="accepDatas" />
		</div>
	</div>
</template>
...
<script>
	...
	// api接口
	import { manaGetStudioRegisterList, reguGetStudioRegisterList, acadGetStudioRegisterList } from "../../../../network/api";
	export default {
		...
		created() {
			...
			// 教务处
			acadGetStudioRegisterList({ unit_id }).then(res => {
				this.tcrDatas = res.data.data;
				this.teacherCount = this.tcrDatas.length;
			});
			// 监管单位
			reguGetStudioRegisterList({ unit_id }).then(res => {
				this.supDatas = res.data.data;
				this.supCount = this.supDatas.length;
			});
			// 直接管理单位
			manaGetStudioRegisterList({ unit_id }).then(res => {
