	...
	<div class="collapse">
		...
		<div>
			<a-modal :title="tanDatas.studio_name" :visible="visible" cancelType="primary" width="60%" @cancel="cancelClick" :centered="true">
				...
				<a-divider>工作室承担实习毕业论文（设计）方案名单</a-divider>
...
<script>
	...
	import { practiceSchemeGetTable, acceptSchemeFeedBackCreditReplace, assumeThesisCheck } from "../../../../../network/api";
	export default {
		props: ["datas"],
		data() {
			return {
		...
		methods: {
			...
			handleClick(text, record, index) {
				...
				practiceSchemeGetTable({ record_id: record.id }).then(res => {
					...
					const arr = [];
					let index = 0;
					for (const iterator of this.tanDatas.apply_content) {
						let obj = iterator;
						obj.id = index;
						arr.push(obj);
						index++;
			...
			handleOk(e) {
				...
				acceptSchemeFeedBackCreditReplace(obj).then(res => {
					this.$emit("againData");
					this.visible = false;
