...
<script>
	...
	// api接口
	import { acceptCreditReplaceDatas, acadCreditReplaceDatas } from "../../../../network/api";
	export default {
		...
		created() {
			...
			// 教务处
			acadCreditReplaceDatas({ unit_id }).then(res => {
				this.tcrDatas = res.data.data;
				this.teacherCount = this.tcrDatas.length;
			});

			// 受理单位
			acceptCreditReplaceDatas({ unit_id }).then(res => {
