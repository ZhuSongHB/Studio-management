...
<script>
	...
	import { creditSchemeGetTable, acceptFeedBackCrediReplace, downloadCreditReplaceScheme, creditReplaceSchemeCheck } from "../../../../../network/api";
	export default {
		props: ["datas"],
		data() {
			return {
		...
		methods: {
			download(record_id) {
				window.open("/api/teacherstudio/download/credit_replace_scheme?record_id=" + record_id);
			},
			handleClick(text, record, index) {
				creditReplaceSchemeCheck({ record_id: record.id }).then(res => {
				...
				creditSchemeGetTable({ record_id: record.id }).then(res => {
			...
			handleOk(e) {
				...
				acceptFeedBackCrediReplace(obj).then(res => {
					this.$emit("againData");
					this.visible = false;
