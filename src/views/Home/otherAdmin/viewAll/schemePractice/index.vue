	...
	<div>
		...
		<div>
			<a-modal :title="tanDatas.studio_name" :visible="visible" cancelType="primary" width="60%" @cancel="cancelClick" @ok="handleOk" :centered="true">
				...
				<a-divider>实习毕业论文（设计）方案名单</a-divider>
				<a-table
					:columns="columns2"
...
<script>
	...
	import { practice_thesis_scheme_get_table } from "../../../../../network/api";
	export default {
		props: ["record"],
		data() {
			return {
		...
		methods: {
			handleClick(text, record, index) {
				practice_thesis_scheme_get_table({ record_id: record.id }).then(res => {
					this.visible = true;
					this.tanDatas = res.data.data;
					this.tableDatas = res.data.data.apply_content;
				});
			},
			cancelClick() {
				// 关闭
