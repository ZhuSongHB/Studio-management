	...
	<div class="collapse">
		...
		<div>
			<a-modal :title="tanDatas.studio_name" :visible="visible" cancelType="primary" width="60%" @cancel="cancelClick" :centered="true">
				...
				<a-divider>毕业论文（设计）名单</a-divider>
				<a-table :columns="columns2" :data-source="tableDatas" :pagination="false" rowKey="student_id" bordered>
					<!-- slot中的name是指scopedSlots中customRender  slot-scope="text"的text就是数据源里的值赋值给text  -->
					<!-- 定义头部名称时 slot等于columns中的slots的title的值，然后赋值 -->
					<span slot="action" slot-scope="text, record, index">
						<a-button @click="handleClick(text, record, index)" type="primary">查看详情</a-button>
...
<script>
	...
	import { assumePracticeApplyTable, acceptFeedbackAssumePractice, assumePracticeCheck, downloadAssumePractice } from "../../../../../network/api";
	export default {
		props: ["datas"],
		data() {
			return {
		...
		methods: {
			download(record_id) {
				window.open("/api/teacherstudio/download/assume_practice?record_id=" + record_id);
			},
			handleClick(text, record, index) {
				assumePracticeCheck({ record_id: record.id }).then(res => {
				...
				assumePracticeApplyTable({ record_id: record.id }).then(res => {
			...
			handleOk(e) {
				...
				acceptFeedbackAssumePractice(obj).then(res => {
					this.$emit("againData");
					this.visible = false;
