...
<script>
	...
	import { assumeThesisApplyGetList, acceptFeedBackCreditReplaceDatas, assumeThesisSheck, downloadAssumeThesis } from "../../../../../network/api";
	export default {
		props: ["datas"],
		data() {
			return {
		...
		methods: {
			...
			handleClick(text, record, index) {
				assumeThesisSheck({ record_id: record.id }).then(res => {
				...
				assumeThesisApplyGetList({ record_id: record.id }).then(res => {
			...
			handleOk(e) {
				...
				acceptFeedBackCreditReplaceDatas(obj).then(res => {
					this.$emit("againData");
					this.visible = false;
